...
PDF_LINKS = {
    "4학년 1학기 수학": "18uznawqJvSEYUGOSqbW4gQ9is6jJ7iuL",
    "6학년 2학기 수학": "11llBJBHbszhvgb7wxB2WANYkNMGaOgYK",
}
...
if menu == "✍️ 학생 문제 풀기":
    ...
    if student_name:
        ...
        if st.session_state.get('exam_started'):
            ...
            if not available_pages:
                ...
                st.info(f"📍 어제까지 {st.session_state['last_solved_id']}번 완료! 오늘 공부할 페이지를 선택하세요.")
                ...
                if st.button("💾 이 페이지 제출 및 진도 저장하기", type="primary"):
                    if teacher_pwd == "0094":
                        ...
                        if not answered_ids:
                            ...
                            st.session_state['last_score_range'] = f"PDF {selected_page}쪽 완료"
    ...
    if not os.path.exists(pdf_file_path): 
        ...
        if saved_ids:
            st.info(f"💡 현재까지 저장된 가장 마지막 번호(ID): {max(saved_ids)}")
        else:
